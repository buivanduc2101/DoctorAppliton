...
public class MoneyAccountActivity extends AppCompatActivity {
    ...
    private void initViews() {
        ...
        progressDialog = new ProgressDialog(MoneyAccountActivity.this);
    ...
    private void setDataToView() {
        tvUsername.setText(userLogin.getPatient().getUser().getUsername());
        tvFullName.setText(userLogin.getPatient().getUser().getFullName());

        if (NetworkUtils.haveNetwork(MoneyAccountActivity.this)) {
            showProgressDialog();

            Call<StatisticMoneyAccount> callLogin = apiService.getStatisticMoneyPatient(userLogin.getPatient().getUser().getId());
            callLogin.enqueue(new Callback<StatisticMoneyAccount>() {
                ...
                public void onResponse(Call<StatisticMoneyAccount> call, Response<StatisticMoneyAccount> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        ...
                        Toast.makeText(MoneyAccountActivity.this, getResources().getString(R.string.co_loi_xay_ra), Toast.LENGTH_SHORT).show();
                    }
            ...
            Toast.makeText(MoneyAccountActivity.this, getResources().getString(R.string.check_connection_network), Toast.LENGTH_SHORT).show();
            hiddenProgressDialog();
        }
    }
